...
local mainMod = "SUPER"
local subMod = "ALT"
local minMod = "ALT + SHIFT"
local miniMod = "SHIFT"

-- Example binds, see https://wiki.hypr.land/Configuring/Basics/Binds/ for more
hl.bind(minMod .. " + E", hl.dsp.exec_cmd(terminal))
local closeWindowBind = hl.bind(minMod .. " + Q", hl.dsp.window.close())
-- closeWindowBind:set_enabled(false)
hl.bind(mainMod .. " + M",
  hl.dsp.exec_cmd("command -v hyprshutdown >/dev/null 2>&1 && hyprshutdown || hyprctl dispatch 'hl.dsp.exit()'"))
hl.bind(mainMod .. " + F", hl.dsp.exec_cmd(fileManager))
hl.bind(mainMod .. " + V", hl.dsp.window.float({ action = "toggle" }))
hl.bind("CONTROL + SPACE", hl.dsp.exec_cmd(menu))
...
hl.bind(subMod .. " + L", hl.dsp.focus({ direction = "right" }))
hl.bind(subMod .. " + K", hl.dsp.focus({ direction = "up" }))
...
-- Switch workspaces with minMod + [0-9]
-- Move active window to a workspace with mainMod + SHIFT + [0-9]
for i = 1, 10 do
  local key = i % 10 -- 10 maps to key 0
  hl.bind(minMod .. " + " .. key, hl.dsp.focus({ workspace = i }))
